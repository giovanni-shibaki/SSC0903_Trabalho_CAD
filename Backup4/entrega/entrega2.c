...
int main(int argc, char *argv[])
{
    ...
    int point[3] = {};
    int *points = (int *)malloc(sizeof(int) * 3 * (num_points_per_node+1));
    ...
    MPI_Bcast(points, 3 * num_points_my_node, MPI_INT, rank, MPI_COMM_WORLD);
    ...
    #pragma omp parallel num_threads(num_threads) private(point, manhattan_euclidean_min_max_point, min_manhattan, max_manhattan, min_euclidean, max_euclidean) default(shared)
    {
        ...
        for (int i = 0; i < num_nodes; i++)
        {
            ...
            MPI_Bcast(points, 3 * num_points_sender, MPI_INT, i, MPI_COMM_WORLD);

            printf("(%d) recebendo de (%d), ponto 1 é (%d, %d, %d)\n", rank, i, points[0], points[1], points[2]);
            ...
            {          
                ...
                point[0] = points[3*j];
                point[1] = points[3*j+1];
                point[2] = points[3*j+2];

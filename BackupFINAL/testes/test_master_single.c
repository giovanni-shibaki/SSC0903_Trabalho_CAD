...
int main(int argc, char *argv[])
{
    ...
    // MPI_Isend(&data_to_send, 1, MPI_INT, 1, TAG, MPI_COMM_WORLD, &send_request);
    // MPI_Irecv(&received_data, 1, MPI_INT, 0, TAG, MPI_COMM_WORLD, &recv_request);
    ...
    #pragma omp parallel num_threads(num_threads)
    {
		...
		printf("Node %d ready to recieve request!\n", rank);
                MPI_Recv(&src, 1, MPI_INT, MPI_ANY_SOURCE, rank, MPI_COMM_WORLD, &status);

                printf("Rank %d received a request from rank %d\n", rank, src);

                MPI_Send(x, (num_points / num_nodes), MPI_INT, src, rank, MPI_COMM_WORLD);
                MPI_Send(y, (num_points / num_nodes), MPI_INT, src, rank, MPI_COMM_WORLD);
                MPI_Send(z, (num_points / num_nodes), MPI_INT, src, rank, MPI_COMM_WORLD);
            }
        }
	...
	#pragma omp single
	{
            for (int i = rank + 1; i < num_nodes; i++)
            {
            	...
            	MPI_Send(&rank, 1, MPI_INT, i, i, MPI_COMM_WORLD);

            	MPI_Recv(x_aux, (num_points / num_nodes), MPI_INT, i, i, MPI_COMM_WORLD, &status);
            	printf("Rank %d recieved X from rank %d\n", rank, i);

            	MPI_Recv(y_aux, (num_points / num_nodes), MPI_INT, i, i, MPI_COMM_WORLD, &status);
            	printf("Rank %d recieved Y from rank %d\n", rank, i);

            	MPI_Recv(z_aux, (num_points / num_nodes), MPI_INT, i, i, MPI_COMM_WORLD, &status);

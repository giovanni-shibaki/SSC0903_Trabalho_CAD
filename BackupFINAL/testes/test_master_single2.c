        ...
        for (int i = rank; i < num_nodes; i++)
        {
            if(i == rank)
            {
                for(int j = 0; j < (num_points_per_node); j++)
                {
                    ...
                    for(int k = j+1; k < (num_points_per_node); k++)
                    {
                        int manhattan = abs2(x[j] - x[k]) + abs2(y[j] - y[k]) + abs2(z[j] - z[k]);
            ...
            else
            {
                ...
                for(int j = 0; j < (num_points_per_node); j++)
                {
                    ...
                    {
                        int manhattan = abs2(x[j] - x_aux[k]) + abs2(y[j] - y_aux[k]) + abs2(z[j] - z_aux[k]);
